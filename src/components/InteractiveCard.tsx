...
const createMetallicTexture = (width: number, height: number) => {
  ...
  for (let i = 0; i < imageData.data.length; i += 4) {
    ...
    imageData.data[i + 1] = Math.min(
      255,
      Math.max(0, imageData.data[i + 1] + rand)
    );
    imageData.data[i + 2] = Math.min(
      255,
      Math.max(0, imageData.data[i + 2] + rand)
    );
  }
    ...
    if (side === "front") {
      ...
      if (project.githubLink) {
        ...
        drawRoundedRect(
          ctx,
          160,
          height - githubButtonHeight - bottomMargin,
          width - 320,
          githubButtonHeight,
          20,
          "rgba(0,0,0,0.8)"
        ...
        ctx.fillText(
          "GitHub",
          width / 2,
          height - githubButtonHeight / 2 - bottomMargin
        );
      }
    } else if (side === "back") {
...
export const InteractiveCard = ({
  ...
  const frontTexture = useMemo(
    () => createCardTexture(project, "front"),
    [project]
  );
  const backTexture = useMemo(
    () => createCardTexture(project, "back"),
    [project]
  );
